...
vec3 calcLighting(int index, vec3 ambient){
    ...
    if(material.hasSpecularMap){
        specular = texture(material.specularMap, TexCoord).rgb * ((spec * material.specular) * lights[index].color);
    } else {
        specular = texture(material.diffuseMap, TexCoord).rgb * ((spec * material.specular) * lights[index].color);
    }
